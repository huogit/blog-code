...
public class VisitLogController {
    ...
    public Result getVisitorList(
        ...
        if(time.equals("")&&uuid.equals("")){
            Page page = new Page(currentPage, pageSize);
            IPage pageData = visitLogService.page(page, new QueryWrapper<VisitLog>()
        ...
        if(time.equals("")){
            ...
            Page page = new Page(currentPage, pageSize);
            IPage pageData = visitLogService.page(page, new QueryWrapper<VisitLog>()
                    .between("create_time", startEndTime[0], startEndTime[1])
                    //.le("create_time", startEndTime[1])
                    //.ge("create_time", startEndTime[0])
                    .orderByDesc("create_time"));
            return Result.succ(pageData);
        }
        else{
            Page page = new Page(currentPage, pageSize);
            IPage pageData = visitLogService.page(page, new QueryWrapper<VisitLog>().eq("uuid",uuid ).le("create_time", startEndTime[1]).ge("create_time", startEndTime[0]).orderByDesc("create_time"));
            return Result.succ(pageData);
        }
