...
export default {
  ...
  methods:{
    ...
    startRead() {
      this.$nextTick(() => {
        this.$emit('fatherMethod') // $emit 方法触发自定义事件
      })
    },
  },
  mounted() {
    ...
    let timer = setTimeout( function fn() { 
      ...
      that.intro = that.intro + str.substring(idx,idx+1) //切割填充字符串
      idx++ //索引加一
      //轮播完，格式化
      if (idx>str.length){ 
        that.intro = ''
    ...
    var waypoint = new Waypoint({
      ...
      handler: function (direction) {
        if(document.getElementById('waypoint')!==null){
          if(document.getElementById('header-photo').style.display !=='none'){ //如果header-photo(父级组件赋予的id，代表本组件)
            if (direction === 'down') {
              $('#nav').show(500);
            } else {
              $('#nav').hide(500);
