<template>
  ...
  <div class="form">
    ...
    <el-form ref="formRef" :model="commentForm" :rules="rules" size="small">
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item prop="nickname"> <!-- 与rule的键对应 -->
        ...
        <el-col :offset="1" :span="5">
          <el-form-item>
            <!-- 接口节流：v-throttle：自定义接口限流：就只是操作时间罢了  -->
...
export default {
  ...
  props: {
    //接收父组件的数据
    realParentCommentId: { //值则是该 prop 预期类型的构造函数
      type: Number,
      default: '-1'
    },
    ...
    return {
      ...
      rules: {
        ...
        email: [
          ...
          {validator: checkEmail} //绑定自定义校验规则
        ],
        website: [
          // 
  ...
  methods: {
    ...
    onInputBlur() {
      ...
      if (!isNaN(Number(qq)) && qq.length > 4 && qq.length < 12) {
        ...
        //根据昵称随机头像
        var randomNum = this.hashFunc(this.commentForm.nickname, 20) //将字符串转成比较大的数字
